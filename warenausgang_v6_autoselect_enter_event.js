    ...
    function setupAutocomplete(inputElement, suggestions) {
        ...
        inputElement.addEventListener("input", function(e) {
            ...
            for (let i = 0; i < suggestions.length; i++) {
                ...
                // Check if the search value exists anywhere in the suggestion
                ...
                if (matchIndex !== -1) {
                    ...
                    // Build the HTML with the matched part in bold
                    let displayText = "";
        ...
        // MODIFIED KEYBOARD NAVIGATION - NOW ENTER SELECTS FIRST ITEM
        // =============================================================
        inputElement.addEventListener("keydown", function(e) {
            ...
            if (e.keyCode == 40) { // Down arrow key (code 40)
                currentFocus++;     // Move highlight down
                addActive(x);       // Update the visual highlight
            } else if (e.keyCode == 38) { // Up arrow key (code 38)
                currentFocus--;     // Move highlight up
                addActive(x);       // Update the visual highlight
            } else if (e.keyCode == 13) { // Enter key (code 13)
                e.preventDefault(); // Stop form from submitting
                
                // KEY CHANGE: NEW LOGIC FOR ENTER KEY
                // ====================================
                if (x && x.length > 0) {  // If there are suggestions showing
                    if (currentFocus > -1) {
                        // If user has highlighted something with arrow keys, select it
                        x[currentFocus].click();
                    } else {
                        // NEW: If nothing is highlighted (currentFocus is -1),
                        // automatically select the FIRST suggestion
                        x[0].click();  // Click the first item (index 0)
                    }
                }
                // If no suggestions are showing, Enter will just do nothing
                // (or you could make it move to next field)
            }
        });
        
        // OPTIONAL: Visual feedback for first item
        // You might want to show the first item as "pre-selected"
        ...
        // Helper function: Remove highlight from all suggestions
        ...
        // Helper function: Close all dropdown lists
